...
`width`和`height`这两个参数说明了场景的宽和高，`deep`为可选参数表示需要的树最高的深度是多少。如果不填的会，会根据最小划分的象限为`16`来推算，`width`和`height`会做pow2对齐，最小不会少于`128`； 这里做对齐主要是为了计算象限直接`/2`来判断，使计算象限的开销降为`O(1)`。
